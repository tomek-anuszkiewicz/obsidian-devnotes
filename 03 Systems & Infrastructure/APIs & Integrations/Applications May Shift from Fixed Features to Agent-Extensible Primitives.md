...
4. **Bring Your Own Data (BYOD)**: User state is migrating out of vendor-locked multi-tenant databases and into standardized, user-controlled local vaults. This operationalizes the architecture behind [[Personal Digital Representation May Become the Foundation of an AI Agent Ecosystem|personal digital representations]]. Applications request scoped read and write access to this shared layer rather than hoarding state.
