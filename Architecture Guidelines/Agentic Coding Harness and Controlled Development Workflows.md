...
An agent can work in a loop, but the loop needs explicit success and failure conditions.

Example:

```text
For the approved step:

1. Add a test describing the required observable behavior.
2. Run it and confirm that it fails for the expected reason.
3. Implement the minimal change.
4. Run the affected module tests.
5. If verification fails, diagnose and correct the cause.
6. Retry no more than five times.
7. Run the broader verification script.
8. Review the final diff against SPEC.md.
9. Stop and present the result.
```
