...
Run the relevant checks before handing off a changed public note. `scripts/check_polish.py` protects the English-only rule, and `scripts/audit_workflow.py` checks language and public wikilinks. Automated checks do not replace a plain-language review.
