...
While human developers are constrained by typing fatigue, AI agents operate with **zero friction**, effortlessly generating unneeded wrapper layers, speculative struct fields, and multi-file sprawl.

Soft guidelines are insufficient against zero-friction generation. The system must enforce **mechanical boundary isolation**:
- **1:1 file hierarchy**: Exactly one file per granular domain operation or command to contain the blast radius.
- **Hard line ceilings (500–800 lines)**: Mechanically enforced by automated linters.
- **Constrained touchpoint budgets**: Hard limits on how many files an agent may modify in a single task.

For the exhaustive analysis of human friction vs. agent generation, localized duplication, and refactoring dynamics, see **[[Software Entropy and the Zero-Friction Trap]]**.
...
- Enforce strict mechanical isolation (1:1 operation-to-file mapping, hard line ceilings like 500–800 lines, and strictly constrained touchpoint budgets) to defeat the zero-friction trap.
...
- Avoid unnecessary internal frameworks and speculative wrapper layers.
