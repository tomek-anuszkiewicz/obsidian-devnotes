...
## Cheap Code and the Power of Extraordinary Questions

Historically, software developers were rarely a company's fundamental competitive moat. The programmer was an essential executor—a skilled, expensive, and high-friction translator turning business intent into machine code. The enduring moat resided in product vision, management decisions, distribution, and domain knowledge.

Now, code generation is virtually free, instantaneous, and commoditized. Any competitor can generate thousands of lines of functional software in minutes using identical frontier models.

When code generation ceases to be a barrier, **where does a company gain an edge?**

```text
The Commodity Trap:
ask average questions → LLM samples Gaussian mean of GitHub → commoditized, identical software

The Non-Obvious Moat:
ask extraordinary questions → force LLM outside training priors → differentiated, high-leverage architecture
```

### 1. The Death of the Implementation Bottleneck
When code was slow to write, companies competed on engineering bandwidth: *"Who can hire 100 engineers to build this platform faster?"*

With coding agents, engineering throughput is democratized. The bottleneck shifts from *writing code* to *framing inquiry*:
- If ten competing firms prompt a model: *"Design an order-processing pipeline,"* they will receive ten nearly identical implementations derived from the statistical average of open-source patterns.
- The company that wins is the one whose architects ask counter-intuitive questions: *"Why do we need a traditional relational database here? What if we modeled this as an append-only log with zero runtime allocations, mechanical 1:1 file isolation, and localized duplicate fast-paths?"*

### 2. Forcing the LLM Outside Established Schemas
LLMs have a strong gravitational pull toward the **Averaged Prior**—the well-trodden, conventional path seen millions of times across public repositories.
- An ordinary prompt yields an ordinary, generic product that any competitor can replicate tomorrow.
...
> When code generation is free, the quality and audacity of an organization's questions become its primary competitive moat.
