...
10. **Executable Architecture Tests as Harness Guardrails**: Architecture tests must test the harness itself, mechanically verifying source line limits ($\le 800$ lines), zero runtime panics, prompt truncation safety thresholds, and golden hash anti-tamper contracts ([[Executable Architecture Tests for Coding Agent Guardrails]]).
