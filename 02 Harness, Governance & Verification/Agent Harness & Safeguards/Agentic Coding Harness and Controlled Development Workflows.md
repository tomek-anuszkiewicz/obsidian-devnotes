...
4. **Use Concrete Tooling for Verification**: Never rely on an LLM to verify its own code through reading or self-reflection when a compiler, linter, or test runner can give an immediate, definitive `PASS / FAIL` signal. The model produces speculative drafts; the test suite verifies reality.
...
6. **Hard Tool-Level Destruction Gating**: File deletion, directory unlinking, and database drops must be physically disabled at the harness API layer or require an out-of-band cryptographic token. Prompts are soft guides; safety requires hard programmatic constraints.
...
8. **Upgrade the Harness, Never Touch the Code**: When an agent errs, resists an invariant, or introduces architectural debt, the engineer must never manually patch lines in the IDE. Fix the root cause in `.agents/rules/`, `.agents/skills/`, or automated architecture tests. Forcing the agent to re-execute against the hardened constraint turns the repository into a self-correcting system.
9. **Active Backlog Pruning & The Living Chronicle**: Accumulating completed tasks in active plan files creates severe prompt bloat and attention degradation. Finished steps must be pruned immediately upon verification ([[Active Backlog Pruning and Context Hygiene in Agentic Roadmaps]]), while detailed narrative history is offloaded to an append-only chronicle (`DIARY.md`) updated via out-of-context tooling ([[The Living Engineering Chronicle - Context-Safe Logging, Evolution, and Compaction]]).
10. **Executable Architecture Tests as Harness Guardrails**: Architecture tests must test the harness itself, programmatically verifying source line limits ($\le 800$ lines), zero runtime panics, prompt truncation safety thresholds, and golden hash anti-tamper contracts ([[Executable Architecture Tests for Coding Agent Guardrails]]).
...
## Architectural decisions vs. routine execution steps

Not every operation should require human approval.

### High-impact architectural steps (Semantic changes)

These change the meaning, risk profile, or architecture of the system and should usually require human confirmation:
...
These can run automatically within an already approved architectural step:
...
The goal is not to micro-manage every test run. The developer should step in to approve changes in meaning, scope, and operational risk.
...
├── DIARY.md                <-- Living Engineering Chronicle (append-only history)
├── .agents/
│   ├── rules/              <-- Invariant rules and negative fences
│   └── skills/             <-- Reusable multi-step operational playbooks
├── tools/
│   ├── log_diary.py        <-- Out-of-context CLI append tool (0 prompt tokens)
│   └── pre_flight.py       <-- Fast local gate verifying formats & architecture
├── docs/
│   ├── architecture/
│   └── workflows/
├── tasks/
...
Maintains the prioritized flight plan for upcoming milestones. Crucially, high-assurance harnesses enforce **Active Backlog Pruning** (see [[Active Backlog Pruning and Context Hygiene in Agentic Roadmaps]]):
- Completed items are **never retained** with `[COMPLETED]` tags or strikethrough text in the active backlog section.
- As soon as a step is verified against all test gates, it is completely deleted from the active roadmap to prevent prompt bloat and context poisoning.
- High-level verified capabilities are summarized in a concise, 1-line bullet under a "Baseline Deliverables" section at the top.

### `DIARY.md` & Out-of-Context Append Tooling

Because the active roadmap prunes completed steps, historical context and evolutionary technical rationale are preserved in a chronological engineering diary (`DIARY.md`; see [[The Living Engineering Chronicle - Context-Safe Logging, Evolution, and Compaction]]).
- Every modification records four mandatory facets: Affected Subsystems, What Changed, Architectural Rationale, and Verification Results.
- **Out-of-Context Append Tooling**: To prevent a 400 KB+ chronicle from consuming the agent's prompt budget, a dedicated CLI script (`python tools/log_diary.py`) appends formatted entries directly to disk in milliseconds with zero prompt ingestion.
- Upon completing major phases, a `compact-diary` skill compresses older chronological entries into high-level architectural digests.
...
- Because current models have few historical training examples of self-governance, developers must set the ground rules—writing explicit instructions, setting boundary limits, and configuring tools.
...
- **Automated Guardrails**: The agent will spot code bloat or runaway edits in a repository and automatically introduce boundary rules (file line limits, touchpoint caps) to govern its own downstream execution.
...
- **Emergence of agent-native patterns**: Future models will naturally lean toward flat, explicit, modular components as their default behavior, because that pattern has proven easier to test, maintain, and verify automatically.
...
Because the model cannot guarantee its own behavioral boundaries, the surrounding harness must enforce **hard technical boundaries**:
...
- **[[Building Determinism from Unpredictable Models - Agent Harness Architecture]]**: Formalizes the host/client control inversion, the 3-tier Pyramid of Control, inner-vs-outer loops, and Gen 3 state-graph orchestration.
...
- **[[Executable Architecture Tests for Coding Agent Guardrails]]**: Native compiler tests enforcing source file line ceilings, zero panics, prompt budget limits, and anti-tamper contracts.
...
- **[[Software Decay and the Hidden Costs of Frictionless AI Code|software entropy and code sprawl]]**: Harness enforcement of programmatic boundaries (file size, touched files) to prevent code sprawl.
